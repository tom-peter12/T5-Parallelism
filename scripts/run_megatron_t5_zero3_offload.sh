...
MASTER_PORT="${MASTER_PORT:-29804}"
NNODES="${NNODES:-${#HOST_ARRAY[@]}}"
NPROC="${NPROC:-1}"

export CUDA_DEVICE_MAX_CONNECTIONS=1

OUTPUT_DIR="${OUTPUT_DIR:-${PROJECT_DIR}/outputs/megatron_zero3_offload}"
CHECKPOINT_PATH="${CHECKPOINT_PATH:-${OUTPUT_DIR}/checkpoints}"
SPLIT_SENTENCES="${SPLIT_SENTENCES:-0}"
DS_CONFIG="${DS_CONFIG:-${PROJECT_DIR}/ds_configs/megatron_zero3_offload.json}"
...
WANDB_RUN_NAME="${WANDB_RUN_NAME:-megatron-zero3-offload-xsum}"
...
echo "=== Megatron T5 ZeRO-3 + CPU Offload (XSum corpus) ==="
...
torchrun \
  ...
  --rdzv_id=t5_megatron_zero3_offload_job \
  --rdzv_backend=c10d \
