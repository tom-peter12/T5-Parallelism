...
MASTER_PORT="${MASTER_PORT:-29704}"
NNODES="${NNODES:-${#HOST_ARRAY[@]}}"
NPROC=1

TASK_NAME="${TASK_NAME:-sst2}"
OUTPUT_DIR="${OUTPUT_DIR:-${PROJECT_DIR}/outputs/zero3_offload}"
...
DS_CONFIG="${PROJECT_DIR}/ds_configs/zero3_offload.json"
WANDB_ENTITY="${WANDB_ENTITY:-t5_mlsys}"
WANDB_PROJECT="${WANDB_PROJECT:-deepseed}"
WANDB_RUN_NAME="${WANDB_RUN_NAME:-zero3-offload-${TASK_NAME}}"

echo "=== T5 DeepSpeed ZeRO-3 Offload ==="
print_summary

torchrun \
  --nnodes="${NNODES}" \
  --nproc_per_node="${NPROC}" \
  --rdzv_id=t5_zero3_offload_job \
  --rdzv_backend=c10d \
