...
MASTER_PORT="${MASTER_PORT:-29703}"
NNODES="${NNODES:-${#HOST_ARRAY[@]}}"
NPROC=1

TASK_NAME="${TASK_NAME:-xsum}"
OUTPUT_DIR="${OUTPUT_DIR:-${PROJECT_DIR}/outputs/zero3}"
...
DS_CONFIG="${PROJECT_DIR}/ds_configs/zero3.json"
WANDB_ENTITY="${WANDB_ENTITY:-t5_mlsys}"
WANDB_PROJECT="${WANDB_PROJECT:-deepseed}"
WANDB_RUN_NAME="${WANDB_RUN_NAME:-zero3-${TASK_NAME}}"

echo "=== T5 DeepSpeed ZeRO-3 ==="
print_summary

torchrun \
  --nnodes="${NNODES}" \
  --nproc_per_node="${NPROC}" \
  --rdzv_id=t5_zero3_job \
  --rdzv_backend=c10d \
